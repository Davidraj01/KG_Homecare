...
export async function createSeoPageAction(
  ...
  const { error } = await supabase.from("seo_pages").insert({
    ...
    section5_heading: readNullableString(formData, "section5_heading"),
    section5_content: readNullableString(formData, "section5_content"),
    faq: parseFaqText(readString(formData, "faq_text")),
    testimonials: parseTestimonialsText(readString(formData, "testimonials_text")),
    other_tags: readNullableString(formData, "head_tags"),
  });
...
export async function updateSeoPageAction(
  ...
  // Hero image handling
  const existingImageUrl = readNullableString(formData, "existing_image_url");
  const shouldRemoveHeroImage = readString(formData, "remove_hero_image") === "true";
  const heroImageFile = formData.get("hero_image") as File | null;
...
export async function deleteSeoPageAction(id: string, imageUrl?: string | null, slug?: string, section2ImageUrl?: string | null) {
  ensureConfigured();
  await requireAdminSession();
  ...
  if (section2ImageUrl) {
    await deleteImage(section2ImageUrl);
  }
