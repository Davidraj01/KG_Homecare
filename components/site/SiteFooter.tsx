...
export function SiteFooter({ seoPages = [] }: { seoPages?: SeoPageLink[] }) {
  return (
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:px-6 md:grid-cols-2 lg:grid-cols-4 lg:px-8">
        ...
        {seoPages.length > 0 && (
          <div className="md:col-span-2 lg:col-span-4">
            <h3 className="text-xs font-bold uppercase tracking-[0.18em] text-white/40">Service Areas</h3>
            <ul className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-sm">
              {seoPages.map((page) => (
                <li key={page.id}>
                  <Link href={`/${page.slug}`} className="text-white/55 hover:text-white">
                    {page.title}
                  </Link>
                </li>
              ))}
